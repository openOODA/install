...
VERSION="$(cat "$(dirname "${BASH_SOURCE[0]:-$0}")/VERSION" 2>/dev/null || curl -sSL --max-time 3 "https://raw.githubusercontent.com/openOODA/install/master/VERSION" 2>/dev/null || echo "0.1.46")"
...
do_install() {
  ...
  # even when all 9 binaries were downloaded and SHA-verified. The
